...
export default function Products({ lang, selectedProductId, initialSearch = '', onNavigate }: ProductsProps) {
  ...
  const augmentedProducts = useMemo(() => getAugmentedProducts(lang), [lang]);

  // Filter products based on search and active categories
  const filteredProducts = useMemo(() => {
    return augmentedProducts.filter(prod => {
  ...
  }, [augmentedProducts, activeCategory, searchQuery, lang]);
  ...
  return (
    <div id="products-catalog-root" className="bg-brand-warm-white text-slate-900 min-h-screen text-left font-sans">
      ...
      <section 
        ...
        style={{
          backgroundImage: `url(${ASSET_HEROES.products})`,
          ...
          {filteredProducts.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {filteredProducts.map((prod) => (
                <div 
                  key={prod.id}
                  className="group bg-brand-cream/15 border border-slate-150 rounded-2xl overflow-hidden shadow-xs hover:border-brand-emerald/40 hover:shadow-md transition-all duration-300 flex flex-col md:flex-row text-left"
                >
                  {/* Left Column: Premium Image */}
                  <div className="md:w-5/12 h-64 md:h-auto overflow-hidden relative bg-white shrink-0 border-b md:border-b-0 md:border-r border-slate-150">
                    <img 
                      src={prod.image} 
                  ...
                  <div className="p-6 flex flex-col justify-between flex-grow space-y-4">
                    <div className="space-y-3.5">
                      ...
                      <h3 className="text-base sm:text-lg font-bold text-brand-forest tracking-tight font-sans">
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-150/40 text-[10px] font-medium text-slate-500 text-left">
                        <div>
                    ...
                    <div className="pt-4 border-t border-slate-150/50">
                      <button 
                        onClick={() => handleOpenDetail(prod)}
                        className="w-full bg-brand-forest hover:bg-brand-emerald text-white text-[10px] font-bold font-mono uppercase tracking-widest py-3 rounded-xl transition-all cursor-pointer text-center font-bold"
                      >
                        {t.view_details}
                      </button>
                    </div>
      ...
      {activeProductDetail && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200 text-left relative">
            
            {/* Close Button */}
            <button 
              onClick={() => setActiveProductDetail(null)}
              className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 rounded-full cursor-pointer text-slate-650 transition-all z-10"
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 p-6 sm:p-8">
              ...
              <div className="md:col-span-5 space-y-4">
                <div className="rounded-xl overflow-hidden border border-slate-150 bg-slate-50 aspect-square">
                  <img 
                ...
                <div className="p-4 bg-brand-cream border border-slate-150 rounded-xl space-y-2 text-left">
              ...
              <div className="md:col-span-7 space-y-6">
                <div className="space-y-2">
                  ...
                  <h2 className="text-xl sm:text-2xl font-extrabold text-brand-forest font-sans">
                ...
                <div className="grid grid-cols-2 gap-3 text-[10px] font-medium text-left">
                  ...
                  <div className="p-3.5 bg-slate-50 border border-slate-150 rounded-xl col-span-2">
                ...
                <div className="space-y-2.5 pt-4 border-t border-slate-150 text-left">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px] font-medium text-slate-700">
                    {(activeProductDetail.specifications[lang] || activeProductDetail.specifications['en'] || []).map((spec: string, sidx: number) => (
                      <div key={sidx} className="flex items-center gap-2 bg-slate-50 border border-slate-150 p-2.5 rounded-lg">
                ...
                <div className="pt-4 flex gap-3">
                  <button 
                    ...
                    className="flex-grow bg-brand-emerald hover:bg-brand-emerald/90 text-white text-[10px] font-bold font-mono uppercase tracking-wider py-4 rounded-xl text-center cursor-pointer transition-all font-bold"
                  >
                    {t.initiate_enquiry}
                  </button>
                  <button 
                    onClick={() => setActiveProductDetail(null)}
                    className="px-5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-bold font-mono uppercase tracking-wider rounded-xl cursor-pointer transition-all font-bold"
                  >
                    {t.close}
                  </button>
                </div>
