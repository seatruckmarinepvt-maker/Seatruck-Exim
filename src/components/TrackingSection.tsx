...
import { Search, Loader2, Thermometer, Droplets, Ship, Calendar, ShieldAlert, CheckCircle, Navigation, Settings, Play } from 'lucide-react';

interface TrackingSectionProps {
  lang: Language;
}

function TrackingSection({ lang }: TrackingSectionProps) {
  ...
  const [simContainer, setSimContainer] = useState<Shipment['containerType']>('Reefer (40ft)');
                ...
                <form onSubmit={handleLaunchSimulator} className="space-y-4 text-xs font-semibold">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <select
                        value={simContainer}
                        onChange={(e) => setSimContainer(e.target.value as Shipment['containerType'])}
                        className="w-full bg-[#0F2D52]/60 border border-white/10 rounded-xl py-2.5 px-3 outline-none text-white font-semibold focus:border-brand-gold/50 cursor-pointer"
                      >
